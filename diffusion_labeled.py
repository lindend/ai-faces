...
model_path = f"diffusion_labeledfaces_{steps}_v{version}.keras"
...
def unet_layer(filters, next_layer):
  def inner(x, labels, time_embedding):
    labels = layers.Dense(units=32)(labels)
    _, w, h, c = x.shape
    mapped_labels = layers.Dense(units=w * h * c)(labels)
    mapped_labels = layers.Reshape(target_shape=(w, h, c))(mapped_labels)
    ...
    if next_layer is not None:
      x = downsample()(x)
      x = next_layer(x, labels, filters * 2, time_embedding)
...
def get_model(num_labels):
  ...
  x = unet_layer(64, sublayers)(image_input, label_input, t_input)
...
class DiffusionMonitor(keras.callbacks.Callback):
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    img_path = f"generated-labeleddiffusion-v{version}"
...
fname = os.path.join("ch12", "list_attr_celeba.txt")
...
diffusion.fit(dataset, callbacks=callbacks, epochs=10000)
